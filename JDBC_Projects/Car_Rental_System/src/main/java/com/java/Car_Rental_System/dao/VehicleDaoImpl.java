...
public class VehicleDaoImpl implements VehicleDao {
	...
	public Vehicle searchByCapacity(int Capacity) throws ClassNotFoundException, SQLException {
		...
		while(rs.next()) {
			veh = new Vehicle();
			veh.setVehicle_ID(rs.getString("Vehicle_ID"));
			veh.setMake(rs.getString("Make"));
			veh.setModel(rs.getString("Model"));
			veh.setDaily_Rate(rs.getInt("Daily_Rate"));
			veh.setStatus(Status.valueOf(rs.getString("Status")));
			veh.setPassenger_Capacity(rs.getInt("Passenger_Capacity"));
			veh.setEngine_Capacity(rs.getString("Engine_Capacity"));
		}
		return veh;
